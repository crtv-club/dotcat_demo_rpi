...
class ShiftRegister(object):
    def __init__(self, data_in, sh_cp, st_cp):  # обьявляем конструктор
        if type(data_in) != int or type(sh_cp) != int or type(
                st_cp) != int:  # проверка на правильность входных значений
    ...
    def clk_puls(self):  # Делаем скачек импульса подавая сначала 0, потом 1, потом снова 0
        ...
        GPIO.output(self.sh_cp, GPIO.HIGH)  # подаем на пин логичискую 1
        GPIO.output(self.sh_cp, GPIO.LOW)  # подаем на пин логичиеский 0

    def shift_reg(self, data):  # Функция работы сдвига регистра
        self.digital_write(GPIO.LOW)
        for i in range(8):  # Задаем диапазон от 0 до 7(по количеству выходов на нашем микроконтроллере, напомню, их 8)
 ...
 #   def synchr(self):  # подаем сигнал синхроизациии(0 - 1 - 0)
   ...
   #     GPIO.output(self.st_cp, GPIO.HIGH)  # Подаем логическую
  #      GPIO.output(self.st_cp, GPIO.LOW)  # Подаем логический 0

    def digital_write(self, status):
        GPIO.output(self.st_cp, status)
